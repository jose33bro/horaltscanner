...
class HoralScannerAPITests(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # Camera alignment pose tests
    # ------------------------------------------------------------------

    def test_camera_pose_pi_homes_then_moves_z(self):
        response = self.client.post("/api/camera/pose/pi")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(data["success"])
        self.assertEqual(data["camera"], "pi")
        # Pi pose: x=0 (no move), y=0 (no move), z=135 → one move_motor call
        self.assertIn(("home_motor", "all"), self.fake_stm32.calls)
        self.assertIn(("move_motor", "z", 135.0), self.fake_stm32.calls)
        # x and y are 0.0 so no move call for them
        self.assertNotIn(("move_motor", "x", 0.0), self.fake_stm32.calls)

    def test_camera_pose_logitech_homes_then_moves_x_and_z(self):
        response = self.client.post("/api/camera/pose/logitech")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(data["success"])
        self.assertEqual(data["camera"], "logitech")
        # Logitech pose: x=20 (180°), y=0, z=135
        self.assertIn(("home_motor", "all"), self.fake_stm32.calls)
        self.assertIn(("move_motor", "x", 20.0), self.fake_stm32.calls)
        self.assertIn(("move_motor", "z", 135.0), self.fake_stm32.calls)

    def test_camera_pose_logitech_x_is_180_degrees(self):
        """Logitech pose must have x=20.0 mm (180° with rotation_distance=40mm)."""
        api_module = self.api_module
        pose = api_module.CAMERA_ALIGNMENT_POSES["logitech"]
        self.assertEqual(pose["x"], 20.0)

    def test_camera_pose_pi_x_is_zero(self):
        """Pi camera pose must have x=0 (mire facing Pi camera at home)."""
        pose = self.api_module.CAMERA_ALIGNMENT_POSES["pi"]
        self.assertEqual(pose["x"], 0.0)

    def test_camera_pose_unknown_camera_returns_400(self):
        response = self.client.post("/api/camera/pose/nikon")
        self.assertEqual(response.status_code, 400)

    def test_camera_pose_no_stm32_returns_503(self):
    ...
    def test_camera_pose_returns_pose_coordinates(self):
        response = self.client.post("/api/camera/pose/logitech")
        self.assertEqual(response.status_code, 200)
        pose = response.get_json()["pose"]
