...
class HoralScannerAPITests(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # Camera calibration pose tests
    # ------------------------------------------------------------------

    def _reset_scan_pose(self):
        """Helper: clear the module-level scan pose."""
        original = self.api_module._scan_pose
        self.api_module._scan_pose = None
        self.addCleanup(setattr, self.api_module, "_scan_pose", original)

    def test_pi_calibration_pose_moves_xy_only(self):
        self._reset_scan_pose()
        response = self.client.post("/api/camera/pi/calibration-pose")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(data["success"])
        self.assertIn("x", data["axes_moved"])
        self.assertIn("y", data["axes_moved"])
        self.assertNotIn("z", data["axes_moved"])
        # Verify motor calls: only x and y were moved
    ...
    def test_usb_calibration_pose_moves_xyz(self):
        self._reset_scan_pose()
        response = self.client.post("/api/camera/usb/calibration-pose")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertTrue(data["success"])
        self.assertIn("x", data["axes_moved"])
        self.assertIn("y", data["axes_moved"])
        self.assertIn("z", data["axes_moved"])

    def test_calibration_pose_unknown_camera_returns_404(self):
        response = self.client.post("/api/camera/unknown/calibration-pose")
        self.assertEqual(response.status_code, 404)

    def test_calibration_pose_no_stm32_returns_503(self):
        original = self.api_module.stm32_driver
        self.api_module.stm32_driver = None
        self.addCleanup(setattr, self.api_module, "stm32_driver", original)
        response = self.client.post("/api/camera/pi/calibration-pose")
        self.assertEqual(response.status_code, 503)

    def test_calibration_pose_memorizes_scan_pose(self):
        self._reset_scan_pose()
        self.client.post("/api/camera/pi/calibration-pose")
        self.assertIsNotNone(self.api_module._scan_pose)
        self.assertEqual(self.api_module._scan_pose["camera"], "pi")

    def test_scan_pose_get_returns_none_before_calibration(self):
        self._reset_scan_pose()
        response = self.client.get("/api/camera/scan-pose")
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(response.get_json()["scan_pose"])

    def test_scan_pose_get_returns_memorized_pose(self):
        self.api_module._scan_pose = {"camera": "pi", "x": 0.0, "y": 0.0, "z": None}
        self.addCleanup(setattr, self.api_module, "_scan_pose", None)
        response = self.client.get("/api/camera/scan-pose")
        self.assertEqual(response.status_code, 200)
        pose = response.get_json()["scan_pose"]
        self.assertEqual(pose["camera"], "pi")

    def test_goto_scan_pose_returns_409_when_no_pose(self):
        self._reset_scan_pose()
        response = self.client.post("/api/camera/scan-pose/goto")
        self.assertEqual(response.status_code, 409)

    def test_goto_scan_pose_moves_motors(self):
        self.api_module._scan_pose = {"camera": "usb", "x": 5.0, "y": 10.0, "z": 50.0}
        self.addCleanup(setattr, self.api_module, "_scan_pose", None)
        response = self.client.post("/api/camera/scan-pose/goto")
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.get_json()["success"])
        moved = [(c[1], c[2]) for c in self.fake_stm32.calls if c[0] == "move_motor"]
        self.assertIn(("x", 5.0), moved)
        self.assertIn(("y", 10.0), moved)
        self.assertIn(("z", 50.0), moved)

    def test_scan_pose_save_stores_current_position(self):
        self._reset_scan_pose()
        self.fake_stm32.status["positions"] = {"x": 2.0, "y": 3.0, "z": 7.5}
        response = self.client.post("/api/camera/scan-pose/save", json={"camera": "usb"})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.get_json()["success"])
        self.assertEqual(self.api_module._scan_pose["camera"], "usb")
        self.assertEqual(self.api_module._scan_pose["x"], 2.0)

    def test_scan_pose_save_unknown_camera_returns_404(self):
        response = self.client.post("/api/camera/scan-pose/save", json={"camera": "bad"})
        self.assertEqual(response.status_code, 404)
        ...
        response = self.client.post("/api/camera/pi/calibration-pose")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["lidar_distance_mm"], 312.5)
